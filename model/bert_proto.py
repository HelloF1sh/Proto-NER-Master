...
class BERT4NER_CRF_PROTONET(BertPreTrainedModel):

    def __init__(self, config, use_crf=False):
        super(BERT4NER_CRF_PROTONET, self).__init__(config)
        self.config = config
        ...
        self.prototypes = self.init_proto()
        self.init_weights()
    ...
    def init_proto(self):
        ...
        """
        res = torch.randn([self.config.num_labels, self.embedding_dim]).cuda()
        return res
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, pred_mask=None, input_labels=None):
        ...
        self.batch_size = sequence_output.size(0)
        self.words_num = sequence_output.size(1)
        sequence_output = self.dropout(sequence_output)
        print(sequence_output.size())
        # (batch_size, max_seq_length, num_labels)
        # emissions = self.classifier(sequence_output)
        emissions = self.batch_dist(inputs=sequence_output,prototypes=self.prototypes).view([self.batch_size, self.words_num, self.config.num_labels])
        if self.use_crf:
