...
from model.bert_proto import BERT4NER_CRF_PROTONET
from model.bilstm_proto import BiLSTM_CRF_PROTO
...
def main(args):
    ...
    if args.model == 'bert':
    ...
    elif args.model == 'bilstm_proto':
        is_BERT = False
        word2id = json.load(open(args.word2id_file, "r", encoding="utf8"))
        model = BiLSTM_CRF_PROTO(len(word2id), args.embedding_dim, args.hidden_dim, args.num_labels, args.hidden_dropout_prob, args.crf)
    else:
        is_BERT = True
        # use 'bert-base-chinese' model
        pretrained_model_name = 'bert-base-chinese'
        tokenizer = BertTokenizer.from_pretrained(pretrained_model_name)
        config = BertConfig.from_pretrained(pretrained_model_name, num_labels=args.num_labels, hidden_dropout_prob=args.hidden_dropout_prob)
        model = BERT4NER_CRF_PROTONET.from_pretrained(pretrained_model_name, config=config, use_crf=args.crf)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='bert', choices=['bert', 'bilstm', "bilstm_proto", "bert_proto"])
    ...
    # file path
    # parser.add_argument('--train_file', type=str, default='./data/few_data/train.txt')
    # parser.add_argument('--dev_file', type=str, default='./data/few_data/dev.txt')
    # parser.add_argument('--test_file', type=str, default='./data/few_data/test.txt')
